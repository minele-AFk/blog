...
import { Play, Pause, SkipBack, SkipForward, Repeat, Shuffle, RefreshCw, ListMusic, Disc3, Volume2, VolumeX, Search, X } from 'lucide-react';
...
export default function MusicClient() {
  ...
  const [indicator, setIndicator] = useState<{ type: 'seek' | 'volume'; direction: 'up' | 'down' | 'forward' | 'backward'; value: number; id: number } | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight') {
        e.preventDefault();
        seekBy(SEEK_STEP);
        setIndicator({ type: 'seek', direction: 'forward', value: SEEK_STEP, id: Date.now() });
      } else if (e.key === 'ArrowLeft') {
        e.preventDefault();
        seekBy(-SEEK_STEP);
        setIndicator({ type: 'seek', direction: 'backward', value: SEEK_STEP, id: Date.now() });
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        const newVol = Math.min(100, (volume || 0) + VOL_STEP);
        setVolume(newVol);
        setIndicator({ type: 'volume', direction: 'up', value: newVol, id: Date.now() });
      } else if (e.key === 'ArrowDown') {
        e.preventDefault();
        const newVol = Math.max(0, (volume || 0) - VOL_STEP);
        setVolume(newVol);
        setIndicator({ type: 'volume', direction: 'down', value: newVol, id: Date.now() });
      } else if (e.key === ' ') {
        e.preventDefault();
        togglePlay();
      }
  ...
  }, [seekBy, togglePlay, volume, setVolume]);
  ...
  }, [currentSong?.id, currentSong?.lyrics, currentSong?.lyric, currentSong?.lrc]);
  ...
  const getPlayModeIcon = () => {
    switch (playMode) {
      ...
      case 'single':
        return <RefreshCw size={18} className="text-indigo-500 md:w-5 md:h-5" />;
      case 'random':
    ...
    return (
      <div className="min-h-screen relative pb-32 flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <span className="font-black text-slate-500 tracking-widest text-sm">{error || '暂无歌曲'}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen relative pb-10 flex flex-col cursor-pointer" onClick={handleBackgroundClick}>
      {/* 背景模糊效果 */}
      ...
      <AnimatePresence>
        {indicator && (
          ...
          >
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/30 dark:bg-white/20 backdrop-blur-md text-white/80 dark:text-white/70 text-xs font-medium border border-white/10 shadow-lg">
              {indicator.type === 'seek' ? (
                <>
                  {indicator.direction === 'backward' ? (
                    <SkipBack size={14} className="opacity-70" />
                  ) : (
                    <SkipForward size={14} className="opacity-70" />
                  )}
                  <span className="tabular-nums">
                    {indicator.direction === 'backward' ? '-' : '+'}{indicator.value}s
                  </span>
                </>
              ) : (
                <>
                  {indicator.direction === 'up' ? (
                    ...
                    <VolumeX size={14} className="opacity-70" />
                  )}
                  ...
                  <span className="tabular-nums text-[10px] opacity-60">{indicator.value}%</span>
                </>
              )}
            </div>
          </motion.div>
      ...
      <div className="w-full max-w-7xl mx-auto mt-24 md:mt-28 px-4 sm:px-6 md:px-10 relative z-10 cursor-default" onClick={handleContentClick}>
          ...
          <div className="md:col-span-5 flex flex-col bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[32px] shadow-2xl p-6 md:p-10 relative overflow-hidden transition-all duration-500 shrink-0 min-h-[460px] sm:min-h-[500px] md:min-h-0">
            <div className="flex-1 flex flex-col items-center justify-center relative z-10 w-full overflow-hidden py-4 md:py-0">
              ...
              <div className="relative w-40 h-40 sm:w-48 sm:h-48 lg:w-64 lg:h-64 flex-shrink-0 aspect-square mb-6 md:mb-10 flex items-center justify-center">
                ...
                >
                  <img src={songCover} alt="cover" className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy" />
            ...
            <div className="w-full mt-auto relative z-20">
              ...
              <div className="w-full flex items-center justify-between px-1 md:px-2 lg:px-4">
                <button onClick={togglePlayMode} className="p-2 transition-transform hover:scale-110">
                  {getPlayModeIcon()}
                </button>
                    ...
                    {showVolumeSlider && (
                      ...
                      >
                        <input
                          ...
                          style={{ background: `linear-gradient(to right, #4f46e5 ${isMuted ? 0 : volume}%, rgba(0, 0, 0, 0.15) 0)` }}
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
                          ...
                          return (
                            <div
                              ...
                              className={`transition-all duration-700 cursor-pointer px-2 md:px-4 rounded-2xl ${
                                isActive
                                  ? 'opacity-100 scale-105 py-2 md:py-3 bg-white/10 hover:bg-white/15'
                                  : 'opacity-20 hover:opacity-60 hover:bg-white/10 hover:scale-[1.02] active:scale-95'
                              }`}
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-2 md:gap-2.5">
                    <AnimatePresence mode="popLayout">
                      {filteredPlaylist.map((song) => {
                        ...
                        const isPlayingThis = originalIndex === currentIndex;
                        return (
                          <motion.div
                            layout
                          ...
                          >
                            <div className="flex items-center gap-3 md:gap-4 w-[85%]">
                              ...
                              <div className="flex flex-col truncate">
                                <span className={`text-sm md:text-[15px] font-black truncate ${isPlayingThis ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-800 dark:text-slate-200'}`}>
